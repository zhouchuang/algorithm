(function(){
    ...
    Base64.fn = Base64.prototype={
        ...
        base64String2Code16:function(base64String){
            ...
            for(var i=0;i<base64String.length;i++){
                tempBinary48.push(this.codeTable[base64String.substring(i,i+1)]);
                if(i%8==7){
                    newString.push(this.binary2Chinese(this.codeTransCode(tempBinary48.join(''),16," ")));
                    tempBinary48 =[];
                }
            }
            return newString.join('');
        },
        code16String2Code6:function(code16String){
            var unCompressBase64String  = [];
            var code48Len = Math.floor(code16String.length/3);
            for(var i=0;i<code48Len;i++){
                var code48Binary = this.chinese2Binary(code16String.substring(i*3,(i+1)*3));
                var fullCode48Binary = "";
                for(var str of code48Binary.split(" ")){
                    fullCode48Binary+='0000000000000000'.substr(str.length)+str;
                }
                for(var j=0;j<8;j++){
                    unCompressBase64String.push(this.decodeTable[fullCode48Binary.substring(j*6,(j+1)*6)]);
                }
            }
            return unCompressBase64String.join('');
        },
    ...
    Base64.decompression=function(code16String){
        return this.prototype.code16String2Code6(code16String);
    },
    Base64.version=1.0,
